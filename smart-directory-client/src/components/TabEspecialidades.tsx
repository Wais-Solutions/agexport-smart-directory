...
interface Specialty { code: string; title: string }
...
export default function TabEspecialidades() {
  ...
  const toggleSpecialty = (spec: Specialty) => {
    setSpecialties(prev =>
      prev.some(s => s.code === spec.code)
        ? prev.filter(s => s.code !== spec.code)
        : [...prev, { code: spec.code, title: spec.title }]
    )
  }
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res  = await fetch(`${API_URL}/specialties/import/${partner.partner_id}`, {
        method: 'POST',
        body: formData,
      })
      ...
      setSpecialties(prev => {
        ...
        const newOnes  = data.imported.filter((s: Specialty) => !existing.has(s.code))
        return [...prev, ...newOnes]
      })
          ...
          <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md mx-4 space-y-5">
            ...
            <div className="text-center space-y-1">
              <p className="font-display text-sm tracking-widest uppercase text-navy font-bold">
                ...
                {importState.phase === 'fetching' && 'Verificando códigos CIE-11...'}
              ...
              <p className="text-xs text-dark/40">
                ...
                {importState.phase === 'done'     && `${importState.imported.length} especialidades importadas correctamente`}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {results.map(r => {
                  ...
                  return (
                    <label key={r.code} className={`flex items-center gap-3 px-4 py-3 cursor-pointer transition-colors ${selected ? 'bg-violet/5' : 'hover:bg-navy/3'}`}>
                      <input type="checkbox" checked={selected} onChange={() => toggleSpecialty(r)} className="accent-violet shrink-0" />
          ...
          <div className="bg-white border border-violet/20 rounded-2xl p-5 shadow-sm space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <p className="text-xs font-display tracking-widest text-navy uppercase font-bold">Especialidades sugeridas</p>
              </div>
                ...
                {suggestions.map(s => {
                  ...
                  return (
                    <label key={s.code} className={`flex items-center gap-3 px-4 py-3 rounded-xl cursor-pointer border transition-all ${selected ? 'bg-violet/8 border-violet/30' : 'bg-navy/3 border-transparent hover:border-violet/20 hover:bg-violet/5'}`}>
                      <input type="checkbox" checked={selected} onChange={() => toggleSpecialty(s)} className="accent-violet shrink-0" />
        ...
        <div className="bg-white border border-navy/10 rounded-2xl p-5 shadow-sm space-y-3 h-fit sticky top-6">
          <div className="flex items-center justify-between">
            ...
            <span className={`text-xs font-display font-bold px-2.5 py-0.5 rounded-full ${specialties.length > 0 ? 'bg-violet text-pearl' : 'bg-navy/5 text-dark/30'}`}>
              {specialties.length}
            </span>
          </div>
          {specialties.length === 0 ? (
            <div className="flex flex-col items-center py-12 gap-3">
              ...
              <p className="text-xs text-dark/30 text-center">Usa el buscador o las sugerencias<br />para agregar especialidades.</p>
            </div>
          ) : (
            <div className="space-y-2 max-h-[60vh] overflow-y-auto pr-1">
              {specialties.map(s => (
                <div key={s.code} className="flex items-center justify-between bg-navy/3 border border-navy/8 rounded-xl px-3 py-2.5 group hover:border-red-200 transition-colors">
                  ...
                  <button onClick={() => toggleSpecialty(s)} className="text-dark/20 hover:text-red-400 transition-colors ml-2 shrink-0 opacity-0 group-hover:opacity-100" title="Eliminar">
                    <X size={14} />
                  </button>
                </div>
              ))}
